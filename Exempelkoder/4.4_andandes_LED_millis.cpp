...
int breathDelay = 50; // Reglerar hur lång tid det tar att andas, tiden att vänta medan värdet ändras
int lastChanged = 0; // Håller koll på när vi senast ändrade värdet på LED:en
...
void loop() {
    int timeNow = millis(); // Tiden nu
    if(timeNow - lastChanged >= breathDelay){ // Om det harr gått tillräckligt med tid sedan vi senast ändrade ljusstyrkan
        lastChanged = millis(); // Uppdatera när den senast ändrades
