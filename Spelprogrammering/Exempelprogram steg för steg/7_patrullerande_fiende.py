...
class Player:
    ...
    """HÄR ÄR DET NYTT"""
    def die(self): # En metod för det som ska göras när spelaren dör. Det kan vara mer än att bara sätta is_alive, tex att den först blinkar
        self.is_alive = False
...
class Enemy:
    ...
    def __init__(self, x=10, y=200):
        ...
        self.y = y
        """HÄR ÄR DET NYTT"""
        self.dx = 1 # sätter hur x ska ändras, här har vi bara att fienden pattrullerar i x-led
        self.dy = 0 # sätter hur y ska ändras
        self.size = 20
        self.speed = 5 # avgör hastigheten som fienden rör sig

    # Efter konstruktorn kommer en update-metod som uppdaterar objektets position
    """HÄR ÄR DET NYTT"""
    def update(self):
        
        if self.x > WIDTH - self.size and self.dx > 0: # om fienden når högra kanten ska den vända
            self.dx = -1
        elif self.x < 0 and self.dx < 0: # om fienden når vänstra kanten ska den vända
            self.dx = 1
        
        self.x += self.dx * self.speed # ändrar fiendens position i x-led enligt hastighet och riktning


    # Därefter har vi en metod som ritar upp objektet
    """HÄR ÄR DET NYTT"""
    def draw(self, screen): # metoden tar in vilken skärrm objektet ska ritas på
        pygame.draw.rect(screen, "red", pygame.Rect(self.x,self.y,self.size,self.size))
...
def main():
    ...
    while running:
        ...
        for i in range(len(candy_list),5): # för så många platser som är kvar i listan med candy, dvs nå många som saknas till 5
            candy_list.append(Candy(random.choice(color_list))) # lägg till en ny godis
